...
# Function to manage SSH tunnel for DocumentDB access
manage_ssh_tunnel() {
    local action="$1"
    local tunnel_port="27018"
    local ec2_host="54.209.241.47"
    ...
    case "$action" in
        "start")
            ...
            ssh -o ExitOnForwardFailure=yes -f -N -L $tunnel_port:$docdb_host:27017 ubuntu@$ec2_host
