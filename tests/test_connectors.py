...
from src.floridify.connectors import WiktionaryConnector, OxfordConnector, DictionaryComConnector
from src.floridify.models import ProviderData, Definition, WordType
...
class TestWiktionaryConnector:
    ...
    async def test_rate_limiting(self, connector: WiktionaryConnector) -> None:
        ...
        # Test with moderate rate limit to avoid long waits
        fast_connector = WiktionaryConnector(rate_limit=10.0)  # 10 per second
        
        start_time = asyncio.get_event_loop().time()
        await fast_connector._enforce_rate_limit()
        await fast_connector._enforce_rate_limit()
        end_time = asyncio.get_event_loop().time()
        
        # Should take at least 0.1 second due to rate limiting
