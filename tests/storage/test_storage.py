...
class TestMongoDBStorage:
    ...
    def storage(self) -> MongoDBStorage:
        ...
        return MongoDBStorage(
            connection_string="mongodb://localhost:27017", database_name="test_floridify"
        )
    ...
    async def connected_storage(self, storage: MongoDBStorage) -> MongoDBStorage:
        ...
        try:
            from src.floridify.models import APIResponseCache, DictionaryEntry
    ...
    def sample_entry(self) -> DictionaryEntry:
        ...
        definition = Definition(
            word_type=WordType.NOUN, definition="A sample definition for testing"
        )
        provider_data = ProviderData(provider_name="test_provider", definitions=[definition])
    ...
    async def test_save_and_retrieve_entry(
        self, connected_storage: MongoDBStorage, sample_entry: DictionaryEntry
    ) -> None:
    ...
    async def test_update_existing_entry(
        ...
        new_definition = Definition(
            word_type=WordType.VERB, definition="A verb definition for testing"
        )
        new_provider_data = ProviderData(
            provider_name="another_provider", definitions=[new_definition]
        )

        updated_entry = DictionaryEntry(
            word=Word(text="test_word"), pronunciation=Pronunciation(phonetic="updated-test")
        )
    ...
    async def test_cache_operations(self, connected_storage: MongoDBStorage) -> None:
        ...
        # Test retrieval
        cached_data = await connected_storage.get_cached_response("test_word", "test_provider")
        assert cached_data == test_data

        # Test cache miss
        missing_data = await connected_storage.get_cached_response("nonexistent", "provider")
    ...
    async def test_cache_age_filtering(self, connected_storage: MongoDBStorage) -> None:
        ...
        # Cache some data
        await connected_storage.cache_api_response("test_word", "test_provider", test_data)
...
async def test_beanie_document_operations() -> None:
    ...
    definition = Definition(word_type=WordType.NOUN, definition="A testing framework")
    provider_data = ProviderData(provider_name="test", definitions=[definition])
