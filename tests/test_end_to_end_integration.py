...
2. Meeting her future business partner at that coffee shop was pure serendipity."""
        ...
        with patch('src.floridify.connectors.wiktionary.WiktionaryConnector.fetch_definition') as mock_wiktionary, \
             patch('src.floridify.connectors.oxford.OxfordConnector.fetch_definition') as mock_oxford, \
             patch('src.floridify.connectors.dictionary_com.DictionaryComConnector.fetch_definition') as mock_dict_com, \
            ...
            wiktionary_provider_data = ProviderData(
                ...
                definitions=[
                    Definition(
                        ...
                        examples=Examples(
                            generated=[
                                GeneratedExample(sentence="A fortunate serendipity brought the two inventors together.", regenerable=False)
                            ]
                        ),
                    ...
                    Definition(
                        ...
                        examples=Examples(
                            generated=[
                                GeneratedExample(sentence="The discovery was a serendipity that changed everything.", regenerable=False)
                            ]
                        ),
                        raw_metadata={"source": "wiktionary"}
                    )
            ...
            oxford_provider_data = ProviderData(
                ...
                definitions=[
                    Definition(
                        ...
                        examples=Examples(
                            generated=[
                                GeneratedExample(sentence="A fortunate stroke of serendipity brought the two old friends together", regenerable=False)
                            ]
                        ),
            ...
            mock_client.chat.completions.create = AsyncMock(side_effect=[synthesis_response, examples_response])
    ...
    async def test_word_processing_with_provider_failures(
        ...
        with patch('src.floridify.connectors.wiktionary.WiktionaryConnector.fetch_definition') as mock_wiktionary, \
            ...
            synthesis_response = MagicMock()
            synthesis_response.choices[0].message.content = "NOUN: AI synthesized definition from available data."
            
            examples_response = MagicMock()
            examples_response.choices[0].message.content = "1. Example sentence one.\n2. Example sentence two."
    ...
    async def test_synonym_mapping_and_database_integration(
        ...
        async def mock_get_entry(word: str) -> DictionaryEntry | None:
            if word == "luck":
        ...
        with patch('src.floridify.connectors.wiktionary.WiktionaryConnector.fetch_definition') as mock_wiktionary, \
            ...
            synthesis_response = MagicMock()
            synthesis_response.choices[0].message.content = "NOUN: The faculty of making fortunate discoveries by accident."
            
            examples_response = MagicMock()
            examples_response.choices[0].message.content = "1. Her serendipity led to a major breakthrough.\n2. The meeting was pure serendipity."
        ...
        with patch('src.floridify.connectors.wiktionary.WiktionaryConnector.fetch_definition') as mock_wiktionary, \
            ...
            def create_ai_response(call_count: list[int]) -> MagicMock:
                ...
                if call_count[0] % 2 == 1:  # Odd calls are synthesis
                    response.choices[0].message.content = f"NOUN: AI definition {call_count[0]//2 + 1}."
                else:  # Even calls are examples
                    response.choices[0].message.content = f"1. Example {call_count[0]//2}.\n2. Another example {call_count[0]//2}."
                return response

            call_count = [0]
            mock_client.chat.completions.create = AsyncMock(side_effect=lambda **kwargs: create_ai_response(call_count))
...
"""
        ...
        assert any("fortunate discoveries" in d.definition for d in noun_definitions)
        assert any("instance of such" in d.definition for d in noun_definitions)
        assert any("psychology" in d.definition for d in noun_definitions)
        
        # Check that examples were extracted
        definitions_with_examples = [d for d in definitions if d.examples.generated]
        assert len(definitions_with_examples) >= 2
