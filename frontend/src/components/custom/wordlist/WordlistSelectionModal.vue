<template>
    <Modal v-model="modelValue" :close-on-backdrop="false">
        <div class="mx-auto w-full max-w-lg space-y-6">
            ...
            <div
                class="rounded-lg border-2 border-dashed border-muted-foreground/30 bg-muted/30 p-4"
            >
            ...
            <div class="space-y-3">
                ...
                <div
                    v-if="userWordlists.length === 0"
                    class="py-8 text-center text-muted-foreground"
                >
                    <p class="text-sm">No wordlists yet.</p>
                    <p class="mt-1 text-xs">Create your first one above!</p>
                </div>
                ...
                <div v-else class="max-h-64 space-y-2 overflow-y-auto">
                    <button
                        ...
                        class="flex w-full items-center justify-between rounded-lg border p-3 text-left transition-colors hover:bg-muted/50 disabled:cursor-not-allowed disabled:opacity-50"
                        :class="{
                            'border-primary bg-primary/5':
                                isWordAlreadyInList(wordlist),
                    ...
                    >
                        <div class="min-w-0 flex-1">
                            <div class="flex items-center gap-2">
                                ...
                                <div
                                    v-if="isWordAlreadyInList(wordlist)"
                                ...
                                <div v-else class="flex items-center gap-0.5">
                                    <div
                                        v-if="
                                            getMasteryStats(wordlist).gold > 0
                                        "
                                        class="h-1.5 w-1.5 rounded-full bg-gradient-to-r from-yellow-400 to-amber-600"
                                        :title="`${getMasteryStats(wordlist).gold} mastered`"
                                    />
                                    <div
                                        v-if="
                                            getMasteryStats(wordlist).silver > 0
                                        "
                                        class="h-1.5 w-1.5 rounded-full bg-gradient-to-r from-gray-400 to-gray-600"
                                        :title="`${getMasteryStats(wordlist).silver} familiar`"
                                    />
                                    <div
                                        v-if="
                                            getMasteryStats(wordlist).bronze > 0
                                        "
                                        class="h-1.5 w-1.5 rounded-full bg-gradient-to-r from-orange-400 to-orange-600"
            ...
            <div v-if="additionHistory.length > 0" class="space-y-2">
                ...
                <div class="text-xs text-muted-foreground">
                    This word has been added to
                    {{ additionHistory.length }} wordlist(s) previously.
                </div>
            </div>
