...
const REGENERABLE_COMPONENTS = new Set([
    'synonyms',
    'antonyms',
    'examples',
    'cefr_level',
...
export function useDefinitionEditMode(
    definition: Ref<
        Definition & { examples: any[]; images: any[]; providers_data: any[] }
    >,
    ...
    const fields = reactive<{
        ...
        cefr_level: EditableField<
            'A1' | 'A2' | 'B1' | 'B2' | 'C1' | 'C2' | null
        >;
        language_register: EditableField<
    ...
    function startEdit() {
        ...
        fields.language_register.value =
            definition.value.language_register ?? null;
    ...
    async function save() {
        ...
        try {
            ...
            if (fields.language_register.isDirty) {
                updateData.language_register =
                    fields.language_register.value || undefined;
            }
            ...
            Object.keys(fields).forEach((key) => {
                ...
                if (Array.isArray(field.value)) {
                    field.originalValue = [...field.value] as any;
                } else if (
                    field.value &&
                    typeof field.value === 'object' &&
                    field.value !== null
                ) {
                    field.originalValue = structuredClone(toRaw(field.value));
                } else {
    ...
    function removeSynonym(index: number) {
        fields.synonyms.value = fields.synonyms.value.filter(
            (_, i) => i !== index
        );
    }
    ...
    async function regenerateComponent(component: string) {
        ...
        try {
            ...
            // The ContentStore's regenerateDefinitionComponent automatically updates currentEntry
            // So we just need to sync our local state if the definition matches currentEntry
            if (contentStore.currentEntry) {
                // Find the updated definition in currentEntry
                const updatedDef = contentStore.currentEntry.definitions?.find(
                    (d) => d.id === definition.value.id
                );
                if (updatedDef) {
                    const newValue = updatedDef[component as keyof Definition];
                    if (newValue !== undefined) {
                        field.value = structuredClone(toRaw(newValue)) as any;
                        field.originalValue = structuredClone(
                            toRaw(newValue)
                        ) as any;
                        field.isDirty = false;
                    }
    ...
    function canRegenerate(component: string): boolean {
        return REGENERABLE_COMPONENTS.has(component);
    }
