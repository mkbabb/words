  ...
  async function regenerateComponent(component: string) {
    ...
    try {
      if (options.onRegenerate) {
        ...
        await contentStore.regenerateDefinitionComponent(definition.value.id, component as any);
      }
      ...
      if (contentStore.currentEntry) {
        ...
        const updatedDef = contentStore.currentEntry.definitions?.find(d => d.id === definition.value.id);
        if (updatedDef) {
          const newValue = updatedDef[component as keyof Definition];
          if (newValue !== undefined) {
            field.value = structuredClone(toRaw(newValue)) as any;
            field.originalValue = structuredClone(toRaw(newValue)) as any;
            field.isDirty = false;
          }
        }
      }
