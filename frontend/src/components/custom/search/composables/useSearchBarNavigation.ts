...
export function useSearchBarNavigation(options: UseSearchBarNavigationOptions) {
    ...
    const scrollToSelectedResult = () => {
        nextTick(() => {
            ...
            const selectedElement =
                searchResultsComponent.value?.resultRefs?.[
                    searchBar.searchSelectedIndex
                ];
        ...
        lookup: {
            ...
            handleEnter: async () => {
                ...
                if (searchBar.isAIQuery && query) {
                    try {
                        ...
                        const wordSuggestions =
                            await orchestrator.getAISuggestions(
                                query,
                                extractedCount
                            );
                        ...
                        if (wordSuggestions?.suggestions?.length > 0) {
                            ...
                            showError(
                                'No word suggestions found for this query'
                            );
                        }
                    } catch (error: any) {
                        ...
                        searchBar.triggerErrorAnimation();
                        showError(
                            error.message || 'Failed to get word suggestions'
                        );
                    }
                    return;
                }
                ...
                if (selectedIndex >= 0 && selectedIndex < results.length) {
                    await modeHandlers.lookup.selectResult(
                        results[selectedIndex] as SearchResult
                    );
                    return;
                }
    ...
    const selectResult = async (result: SearchResult) => {
        const handler =
            modeHandlers[searchBar.searchMode as keyof typeof modeHandlers];
