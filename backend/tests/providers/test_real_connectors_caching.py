...
from floridify.providers.dictionary.models import DictionaryProviderEntry as DictionaryEntryMetadata
...
from floridify.providers.literature.models import (
    LiteratureEntry as LiteratureEntryMetadata,
    LiteratureSource,
)
...
class TestRealWiktionaryConnector:
    ...
    async def test_definition_creation_and_linking(self, test_db):
        ...
        definition = Definition(
            word_id=word.id, part_of_speech="noun", text="A test definition", sense_number="1"
        )
...
class TestRealVersioning:
    ...
    async def test_literature_entry_versioning(self, test_db):
        ...
        content_v2 = {
            ...
            "chapters": ["Chapter 1: The beginning.", "Chapter 2: The continuation."],
            "word_count": 8,
        }
...
class TestRealCacheVersioningIntegration:
    ...
    async def test_cache_invalidation_on_version_update(self, test_db):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Cache v1
            cache_key = f"dict_entry:{word.text}:latest"
            v1_cache_data = {"version": "1.0.0", "content": "Version 1 content", "is_latest": True}
            ...
            # Cache v2
            v2_cache_data = {"version": "2.0.0", "content": "Version 2 content", "is_latest": True}
