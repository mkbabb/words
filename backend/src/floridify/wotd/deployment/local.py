...
class LocalWOTDServer:
    ...
    async def _generate_from_semantic_id(
        self, semantic_id: tuple[int, int, int, int], num_words: int
    ) -> list[str]:
        ...
        if self.semantic_ids:
            closest_corpus_id = min(
                ...
                key=lambda k: sum(abs(a - b) for a, b in zip(self.semantic_ids[k], semantic_id))
            )
