...
class SyntheticGenerator:
    ...
    async def generate_corpus(
        ...
        if author:
            ...
            authorial_flavor = author_influences.get(author, f"with {author.value}'s distinctive style")
    ...
    async def generate_complete_training_set(
        ...
        for i in range(0, len(combinations), batch_size):
            ...
            for j, result in enumerate(batch_results):
                if isinstance(result, Exception):
                    ...
                    logger.error(f"❌ Failed to generate {style}/{complexity}/{era}{author_info}: {result}")
                else:
        ...
        if save:
            ...
            await storage.save_multiple_corpora(list(corpora.values()))
