...
class SearchEngine:
    ...
    def __init__(
        ...
        vocabulary_source: Lexicon | None = None,
    ) -> None:
        """
        ...
        Args:
            ...
            vocabulary_source: Optional custom vocabulary source (overrides lexicon loading)
        """
    ...
    async def initialize(self) -> None:
        ...
        # Load vocabulary from custom source or lexicon
        if self.vocabulary_source:
            # Use custom vocabulary source
            logger.info("Using custom vocabulary source")
            self._words = self.vocabulary_source.get_all_words()
            self._phrases = self.vocabulary_source.get_all_phrases()
            self._vocabulary = self._words + self._phrases
        else:
            # Load lexicons (legacy behavior)
    ...
    async def create_with_vocabulary(
        cls,
        vocabulary_source: Lexicon,
        min_score: float = 0.6,
        enable_semantic: bool = False,
    ) -> "SearchEngine":
        """
        Create and initialize a SearchEngine with custom vocabulary.
        
        Args:
            vocabulary_source: Custom vocabulary source
            min_score: Minimum relevance score for results
