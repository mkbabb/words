...
class CorpusData(Document, BaseMetadata):
    ...
    semantic_index_id: PydanticObjectId | None = Field(
        None, description="Reference to semantic index"
    )
...
class CorpusCacheEntry(Document, BaseMetadata):
    ...
    # Identification
    language: Language = Field(..., description="Language of corpus")
    ...
    @classmethod
    async def get_cached_corpus(
        cls, language: str | Language, source_hash: str
    ) -> CorpusCacheEntry | None:
        """Get cached corpus data by language and source hash."""
        # Handle both string and Language enum
        lang_value = language.value if isinstance(language, Language) else language
...
class CorpusCompressionUtils:
    ...
    def decompress_data(compressed_data: bytes) -> bytes:
        ...
        try:
            ...
            result: bytes = decompressor.decompress(compressed_data)
            return result
        except ImportError:
            # Fallback to zlib
...
class SemanticIndexCache(Document, BaseMetadata):
    ...
    quantization_type: QuantizationType = Field(
        default=QuantizationType.NONE, description="Type of quantization applied"
    )
