...
class TrieSearch:
    ...
    def search_exact(self, query: str) -> str | None:
        ...
        1. Normalize query for case-insensitive search
        2. Bloom filter (fast negative check - ~50-100ns)
        3. Trie lookup (only if Bloom says "maybe" - ~500-1000ns)

        This gives ~30-50% speedup for non-existent words (the common case).

        Args:
            query: Query string to search for (will be normalized)

        Returns:
            The normalized query if found, None otherwise
        ...
        if self._bloom_filter and normalized_query not in self._bloom_filter:
            # Definitely not in vocabulary - return immediately
            return None

        # OPTIMIZATION 2: Use marisa-trie's optimized exact search (C++ backend)
