...
class WordOfTheDayEntry(BaseModel):
    ...
    generated_at: datetime = Field(default_factory=datetime.now, description="When this entry was generated")
    sent_at: datetime | None = Field(default=None, description="When this entry was sent to users")


class WordOfTheDayBatch(Document):
    ...
    frequency: NotificationFrequency = Field(default=NotificationFrequency.DAILY, description="Notification frequency")
    ...
    # Word pools
    current_batch: list[WordOfTheDayEntry] = Field(default_factory=list, description="Current batch of unsent words")
...
class WordOfTheDayConfig(Document):
    """Global configuration for Word of the Day system."""
    
    default_batch_size: int = Field(default=20, ge=5, le=100, description="Default batch size for generation")
    min_batch_threshold: int = Field(default=5, ge=1, le=20, description="Generate new batch when below this threshold")
    max_previous_words: int = Field(default=100, ge=10, le=1000, description="Max previous words to send to AI")
    generation_enabled: bool = Field(default=True, description="Whether to generate new batches automatically")
