"""Wordlist command for batch dictionary operations."""
...
async def _create_async(
    ...
    wordlist_repo = WordListRepository()

    # Check if word list already exists
    existing = await wordlist_repo.find_by_name(name)
    if existing:
    ...
    # Save word list first to get an ID
    ...
    await wordlist_repo.recompute_stats(word_list.id)
...
async def _show_async(name: str, num: int | None) -> None:
    ...
    word_list = await wordlist_repo.find_by_name(name)
    if not word_list:
...
async def _list_async() -> None:
    ...
    word_lists, _ = await wordlist_repo.list()
    if not word_lists:
...
async def _update_async(name: str, input_file: Path) -> None:
    ...
    for wid in word_ids:
        ...
        if existing:
            ...
            await WordListItemDoc(wordlist_id=word_list.id, word_id=wid).insert()
...
async def _delete_async(name: str) -> None:
    ...
    await wordlist_repo.delete(word_list.id, cascade=True)
