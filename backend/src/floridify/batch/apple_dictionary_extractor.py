...
class AppleDictionaryBatchExtractor:
    ...
    def __init__(self, config: ExtractionConfig, mongodb: MongoDBStorage | None = None) -> None:
    ...
    async def extract_word_list(self, words: list[str]) -> list[ProviderData]:
        ...
        if self.config.log_progress:
            ...
            task_id = progress.add_task(
                "[green]Extracting Apple Dictionary definitions...", total=len(words)
            )
...
async def extract_common_words(
    word_count: int = 1000, output_file: Path | None = None, mongodb: MongoDBStorage | None = None
) -> list[ProviderData]:
    ...
    config = ExtractionConfig(output_file=output_file, save_to_mongodb=mongodb is not None)
...
async def extract_from_search_engine(
    search_limit: int = 1000, mongodb: MongoDBStorage | None = None
) -> list[ProviderData]:
