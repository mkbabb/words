...
class BatchProcessor:
    ...
    async def get_unprocessed_words(self, all_words: list[str]) -> list[str]:
        ...
        if hasattr(self.mongodb, "client") and self.mongodb.client:
            try:
                ...
                console.print(
                    f"[blue]Found {len(synthesized_words):,} already synthesized words[/blue]"
                )
    ...
    def create_batch_jobs(self, words: list[str]) -> list[BatchJob]:
        ...
        console.print(
            f"[green]Created {len(jobs)} batch jobs ({self.config.batch_size} words each)[/green]"
        )
        return jobs
    ...
    async def fetch_definitions_for_word(self, word: str) -> list[Definition]:
        ...
        search_results = await self.search_engine.search(word, max_results=1)
        ...
        for provider in self.config.providers:
            try:
                ...
                console.print(
                    f"[yellow]Warning: Provider {provider.__class__.__name__} failed for '{word}': {e}[/yellow]"
                )

        return definitions

    def create_cluster_mapping_request(
        self, word: str, definitions: list[Definition], custom_id: str
    ) -> dict[str, Any]:
    ...
    def create_synthesis_request(
        self, word: str, cluster_definitions: list[Definition], cluster_id: str, custom_id: str
    ) -> dict[str, Any]:
    ...
    async def prepare_batch_file(self, job: BatchJob) -> str:
        ...
        with Progress() as progress:
            ...
            for word in job.words:
                ...
                if self.config.enable_clustering:
                    cluster_request = self.create_cluster_mapping_request(
                        word, definitions, custom_id
                    )
                ...
                if self.config.enable_synthesis:
                    synth_request = self.create_synthesis_request(
                        word, definitions, "default", custom_id
                    )
        ...
        console.print(
            f"[green]Created batch file with {len(batch_requests)} requests: {batch_file_path}[/green]"
        )
        ...
        with Progress() as progress:
            ...
            for job in jobs:
                if job.openai_batch_id:
                    tasks[job.openai_batch_id] = progress.add_task(f"Job {job.job_id}", total=100)
            ...
            while any(job.status == BatchJobStatus.PROCESSING for job in jobs):
                    ...
                    try:
                        ...
                        if batch_status.status == "completed":
                            ...
                            if batch_status.output_file_id:
                                job.output_file_path = await self.download_results(
                                    job, batch_status.output_file_id
                                )
                ...
                for job in jobs:
                    if (
                        job.status == BatchJobStatus.PROCESSING
                        and job.started_at
                        and datetime.now() - job.started_at
                        > timedelta(hours=self.config.processing_timeout_hours)
                    ):
                        job.status = BatchJobStatus.FAILED
    ...
    async def run_batch_processing(self, word_limit: int | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            summary = {
                ...
                "success_rate": (total_processed / len(filtered_words)) * 100
                if filtered_words
                else 0,
            }
    ...
    def _display_summary(
        self, summary: dict[str, Any], completed_jobs: list[BatchJob], failed_jobs: list[BatchJob]
    ) -> None:
        ...
        if summary["total_words_processed"] > 0:
            ...
            ) * 0.075  # $0.075/1M tokens for GPT-4o-mini batch
