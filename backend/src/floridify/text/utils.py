...
IDIOM_PATTERN = re.compile(
    r"\b(?:out of|in order to|as well as|in spite of|on behalf of)\b", re.IGNORECASE
)  # Common English idioms
COLLOCATION_PATTERN = re.compile(
    r"\b(?:strongly|highly|deeply)\s+(?:recommend|suggest|believe)\b", re.IGNORECASE
)  # Collocation patterns
PREPOSITIONAL_PATTERN = re.compile(
    r"\b(?:in|on|at|by|for|with|under|over)\s+(?:the|a|an)?\s*\w+(?:\s+\w+)*\b", re.IGNORECASE
