...
def compress_data(data: bytes, compression_type: CompressionType = CompressionType.ZLIB) -> tuple[bytes, CacheMetadata]:
    """
...
def quantize_embeddings(
    ...
    if quantization_type == QuantizationType.FLOAT32:
        ...
        min_val, max_val = embeddings.min(), embeddings.max()
        if max_val > min_val:
            quantized_embeddings = ((embeddings - min_val) / (max_val - min_val) * 255).astype(np.uint8)
        else:
    ...
    quantized_data = pickle.dumps({
        ...
        'original_range': (embeddings.min(), embeddings.max()) if quantization_type in (QuantizationType.INT8, QuantizationType.UINT8) else None
    })
...
def dequantize_embeddings(quantized_data: bytes, metadata: CacheMetadata) -> np.ndarray:
    ...
    if quantization_type == QuantizationType.FLOAT32:
        ...
        return quantized_embeddings.astype(np.float32)  # Convert back to float32 for processing
    elif quantization_type == QuantizationType.INT8:
        # Restore original scale
        if original_range:
            min_val, max_val = original_range
            scale = max(abs(min_val), abs(max_val))
            return (quantized_embeddings.astype(np.float32) / 127.0 * scale)
        return quantized_embeddings.astype(np.float32)
    elif quantization_type == QuantizationType.UINT8:
        # Restore original range
        if original_range:
            min_val, max_val = original_range
            return (quantized_embeddings.astype(np.float32) / 255.0 * (max_val - min_val)) + min_val
        return quantized_embeddings.astype(np.float32)
    else:
        raise ValueError(f"Unsupported quantization type: {quantization_type}")
