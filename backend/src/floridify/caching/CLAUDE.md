...
3-tier cache. L1 memory (OrderedDict LRU) -> L2 disk (DiskCache) -> L3 versioned MongoDB (SHA-256 content-addressable). Delta compression via DeepDiff.
...
├── delta.py (132)           # DeepDiff-based delta versioning
├── compression.py (69)      # ZSTD, LZ4, GZIP
├── validation.py (183)      # Version validation, corruption detection
├── version_chains.py (143)  # Semantic versioning, chain management
└── utils.py (117)           # JSON encoder, namespace normalization, run_in_executor
```
