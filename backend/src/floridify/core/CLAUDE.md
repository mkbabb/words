...
```
core/
├── lookup_pipeline.py      # 5-stage lookup orchestration
├── search_pipeline.py      # SearchEngineManager + cascade
├── state_tracker.py        # Real-time progress, multi-subscriber
├── streaming.py            # SSE: chunked responses, heartbeat
...
Searches first to normalize the word form, then uses best match for provider fetch. `_create_provider_mapped_entry()` for no_ai mode (dedup+cluster only). `_ai_fallback_lookup()` when all providers fail.
