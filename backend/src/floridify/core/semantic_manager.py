...
class SemanticSearchManager:
    ...
    async def get_or_create_semantic_search(
        ...
        logger.info(
            f"Creating semantic search for corpus '{corpus_name}' with {len(vocabulary)} words"
        )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "memory_usage": {
                name: {
                    "vocabulary_size": len(instance.vocabulary)
                    if hasattr(instance, "vocabulary") and instance.vocabulary
                    else 0,
