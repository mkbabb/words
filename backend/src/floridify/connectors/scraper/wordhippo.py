...
class WordHippoConnector(DictionaryConnector):
            ...
            for section in word_type_sections:
                ...
                for idx, def_div in enumerate(def_divs):
                    ...
                    if definition.id is not None:
                        examples = await self._extract_examples_for_definition(parent, definition.id)
                        if examples:
    ...
    async def _fetch_sentences(self, word: str) -> list[str]:
        ...
        try:
            ...
            sentence_elements = soup.find_all(["div", "span"], class_=["sentence", "example", "examplesentence"])
    ...
    async def _enhance_definitions_with_data(
        ...
        try:
            ...
            for i, definition in enumerate(definitions):
                ...
                end_idx = ((i + 1) * len(synonyms)) // len(definitions) if definitions else len(synonyms)
                ...
                end_idx = ((i + 1) * len(antonyms)) // len(definitions) if definitions else len(antonyms)
                ...
                end_idx = ((i + 1) * len(sentences)) // len(definitions) if definitions else len(sentences)
