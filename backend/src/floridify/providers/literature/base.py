...
from ...models.versioned import CorpusVersionedData, StorageType
...
class LiteratureConnector(BaseConnector):
    ...
    async def _get_existing_work(self, source_id: str) -> LiteratureMetadata | None:
        ...
        resource_id = f"literature_{self.provider_name}_{source_id}"
        versioned = await manager.get_latest(resource_id, resource_type="corpus")
    ...
    async def _create_and_save_metadata(
        ...
        literature_metadata.quality_metrics = literature_metadata._calculate_quality_metrics(
            content
        )
    ...
    async def _save_versioned_data(
        ...
        manager = VersionedDataManager[CorpusVersionedData]()
        resource_id = f"literature_{self.provider_name}_{literature_metadata.source_id}"
        ...
        await manager.save(
            ...
            content={
                ...
                "work_id": str(literature_metadata.id),
                "language": str(literature_metadata.language),
                "provider_name": self.provider_name,
                "text_hash": literature_metadata.text_hash,
                "content_location": literature_metadata.content_location.model_dump()
                if literature_metadata.content_location
                else None,
                "metadata": metadata,
            },
