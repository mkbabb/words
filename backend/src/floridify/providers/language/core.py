...
from ...providers.language.models import LanguageProvider, ParserType, ScraperType
...
from .scraper.scrapers import default_scraper, scrape_french_expressions
...
ParserFunc = Callable[..., tuple[list[str], list[str]]]

# Map scraper enums to functions
SCRAPER_MAP: dict[ScraperType, ScraperFunc] = {
    ScraperType.DEFAULT: default_scraper,
    ScraperType.FRENCH_EXPRESSIONS: scrape_french_expressions,
}

# Map parser enums to functions
PARSER_MAP: dict[ParserType, ParserFunc] = {
    ParserType.TEXT_LINES: parse_text_lines,
    ParserType.JSON_VOCABULARY: parse_json_vocabulary,
    ParserType.CSV_WORDS: parse_csv_words,
}
...
class LanguageConnector(BaseConnector):
    ...
    async def get(
        ...
        """Get language entry from versioned storage."""
    ...
    async def save(
        ...
        """Save language entry to versioned storage."""
        manager = get_version_manager()
        full_resource_id = f"{resource_id}_{self.provider.value}"
        ...
        logger.info(f"Saved language entry for '{resource_id}' from {self.provider.display_name}")
    ...
    async def fetch(
        ...
        """Fetch language entry from provider and optionally save."""
        config = config or VersionConfig()
