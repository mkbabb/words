        ...
        for endpoint, times in self.request_times.items():
            if times:
                ...
                request_stats[endpoint] = {
                    ...
                    "p95_ms": sorted(durations)[int(len(durations) * 0.95)] * 1000 if len(durations) > 20 else None,
                }
    ...
    def decorator(func: Callable) -> Callable:
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if duration > 0.1:
                    logger.warning(
                        f"Slow DB query: {collection}.{operation} took {duration:.3f}s"
                    )
...
class SlowQueryDetector:
    ...
    async def track(self, query_description: str):
        ...
        if duration_ms > self.threshold_ms:
            ...
            logger.warning(
                f"Slow query detected: {query_description} took {duration_ms:.2f}ms"
            )
