...
class DeduplicationMetrics:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "deduplication_rate": (
                self.deduplicated_requests / self.total_requests if self.total_requests > 0 else 0
            ),
            "cache_hit_rate": (
...
def custom_dedup_key_example() -> dict[str, Any]:
    ...
    def simple_key(user_id: str) -> str:
        return f"user:{user_id}"

    # 2. Composite key from multiple parameters
    def composite_key(user_id: str, resource_id: str) -> str:
        return f"user:{user_id}:resource:{resource_id}"

    # 3. Key that ignores certain parameters
    def selective_key(
        word: str, include_etymology: bool = False, debug: bool = False
    ) -> tuple[str, str, bool]:
