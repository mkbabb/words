    ...
    async def remove_object_id_from_arrays(
        ...
        tasks = [
            collection.update_many(
                {array_field: object_id},
                {"$pull": {array_field: object_id}}
            )
        ...
        logger.warning(f"Removed ObjectId {object_id} from {total_modified} documents across {len(collections)} collections")
