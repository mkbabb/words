...
class WordListEntryInput(BaseModel):
    """Structured word entry input for create/add/upload flows."""

    source_text: str = Field(..., min_length=1, description="Original source text")
    resolved_text: str | None = Field(default=None, description="Canonical resolved word")
    frequency: int = Field(default=1, ge=1, description="Occurrence count")
    notes: str | None = Field(default=None, description="Optional notes")
    tags: list[str] = Field(default_factory=list, description="Optional per-entry tags")
...
class WordListCreate(BaseModel):
    ...
    words: list[str | WordListEntryInput] = Field(
...
WordListCreate.model_rebuild()
WordAddRequest.model_rebuild()
...
class WordListRepository(BaseRepository[WordList, WordListCreate, WordListUpdate]):
    ...
    @staticmethod
    def _coerce_entries(
        words: list[str | WordListEntryInput],
    ) -> list[WordListEntryInput]:
        entries: list[WordListEntryInput] = []
        for item in words:
            if isinstance(item, WordListEntryInput):
                entries.append(item)
            else:
                entries.append(WordListEntryInput(source_text=item))
        return entries
    ...
    def _collapse_entries(
        ...
        collapsed: dict[str, WordListEntryInput] = {}

        for entry in WordListRepository._coerce_entries(words):
            text = (entry.resolved_text or entry.source_text).strip()
            normalized = normalize(text)
            existing = collapsed.get(normalized)
            if existing:
                ...
                collapsed[normalized] = WordListEntryInput(
    ...
    @staticmethod
    def _hash_words(words: list[str | WordListEntryInput]) -> list[str]:
        hash_words: list[str] = []
        for entry in WordListRepository._collapse_entries(words):
            text = entry.resolved_text or entry.source_text
            hash_words.extend([text] * max(1, entry.frequency))
        return hash_words
    ...
    async def create(self, data: WordListCreate) -> tuple[WordList, bool]:
        ...
        collapsed_entries = self._collapse_entries(data.words)
        hash_id = generate_wordlist_hash(self._hash_words(data.words))
        ...
        if collapsed_entries:
            ...
            items = [
                ...
                for wid, entry in zip(word_ids, collapsed_entries, strict=False)
            ]
    ...
    async def remove_word(
        ...
        await WordListItemDoc.find({"wordlist_id": wordlist_id, "word_id": word_id}).delete()
    ...
    async def get_statistics(self, wordlist_id: PydanticObjectId) -> dict[str, Any]:
        ...
        Delegates to the wordlist_stats service for aggregation logic.
        """
        from ..services.wordlist_stats import get_statistics

        return await get_statistics(wordlist_id)
    ...
    async def _finalize_word_change(self, wordlist_id: PydanticObjectId) -> WordList:
        """Recompute stats, mark accessed, and update corpus vocabulary after a word change."""
        await self.recompute_stats(wordlist_id)
        ...
        # Update corpus vocabulary incrementally instead of invalidating
        corpus_manager = await self._get_tree_corpus_manager()
        corpus_name = f"wordlist_{wordlist_id}"
        corpus = await corpus_manager.get_corpus(corpus_name=corpus_name)
        if corpus:
            word_texts = await self._get_word_texts_for_wordlist(wordlist_id)
            corpus.vocabulary = word_texts
            corpus.update_version("vocabulary updated after word change")
            await corpus_manager.save_corpus(corpus=corpus)
        else:
            # Corpus doesn't exist yet, create it
            await self._create_wordlist_corpus(wordlist)
    ...
    async def recompute_stats(self, wordlist_id: PydanticObjectId) -> None:
        """Recompute denormalized stats on WordList.

        Delegates to the wordlist_stats service for aggregation logic.
        """
        from ..services.wordlist_stats import recompute_stats

        await recompute_stats(wordlist_id)
