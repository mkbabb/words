...
async def update_word_field(
    ...
    result = await Word.get_motor_collection().find_one_and_update(
        ...
        {
            "$set": {update.field: update.value, "updated_at": datetime.utcnow()},
            "$inc": {"version": 1},
        },
...
async def update_definition_field(
    ...
    result = await Definition.get_motor_collection().find_one_and_update(
        {"_id": definition_id, "version": update.version},
        {
            "$set": {update.field: update.value, "updated_at": datetime.utcnow()},
            "$inc": {"version": 1},
        },
...
class BatchAtomicUpdate(BaseModel):
    ...
    updates: list[dict[str, Any]] = Field(
        ..., description="List of atomic updates to perform", max_items=100
    )
...
async def batch_atomic_updates(
    ...
    for update in request.updates:
        if "word_id" in update:
            ...
            word_operations.append(
                UpdateOne(
                    {"_id": PydanticObjectId(update["word_id"]), "version": update["version"]},
                    {
                        "$set": {update["field"]: update["value"], "updated_at": datetime.utcnow()},
                        "$inc": {"version": 1},
                    },
    ...
    if word_operations:
        word_result = await Word.get_motor_collection().bulk_write(word_operations, ordered=False)
    ...
    if definition_operations:
        def_result = await Definition.get_motor_collection().bulk_write(
            definition_operations, ordered=False
        )
