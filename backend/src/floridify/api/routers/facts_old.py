...
class FactParams(BaseModel):
    ...
    count: int = Field(default=5, ge=3, le=8, description="Number of facts to generate")
...
def parse_fact_params(
    ...
    previous_words: list[str] | None = Query(
        default=None, description="Previously searched words for context"
    ),
) -> FactParams:
...
@cached_api_call(
    ...
    key_func=lambda word, params: (
        ...
        word,
        params.count,
        tuple(sorted(params.previous_words)) if params.previous_words else None,
    ),
)
async def _cached_facts(word: str, params: FactParams) -> FactsAPIResponse:
    ...
    try:
        ...
        if not entry:
            raise HTTPException(
                status_code=404, detail=f"Word '{word}' not found. Please lookup the word first."
            )
        ...
        raise HTTPException(status_code=500, detail=f"Internal error generating facts: {str(e)}")
