...
class RegenerateRequest(BaseModel):
    ...
    components: set[str] = Field(
        default_factory=lambda: {"all"}, description="Components to regenerate"
    )
...
async def get_entry(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid entry ID format: {entry_id}"
        )
    ...
    try:
        ...
        return ResourceResponse(
            ...
            metadata={
                ...
                if updated_entry.updated_at
                else None,
            },
        )
    except NotFoundException:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Entry with ID {entry_id} not found"
        )
    except ValueError:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid entry ID format: {entry_id}"
...
async def delete_entry(
    entry_id: str, repo: SynthesisRepository = Depends(get_synthesis_repo)
) -> ResourceResponse:
    ...
    try:
        ...
        return ResourceResponse(data={"deleted": True}, metadata={"id": entry_id})
    except NotFoundException:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Entry with ID {entry_id} not found"
        )
...
async def add_images_to_entry(
    entry_id: str, request: ImageAddRequest, repo: SynthesisRepository = Depends(get_synthesis_repo)
) -> ResourceResponse:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid ID format: {str(e)}"
        )


@router.delete("/{entry_id}/images/{image_id}")
async def remove_image_from_entry(
    entry_id: str, image_id: str, repo: SynthesisRepository = Depends(get_synthesis_repo)
) -> ResourceResponse:
