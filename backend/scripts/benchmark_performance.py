...
TEST_QUERIES = {
    "common": ["test", "word", "example", "dictionary", "meaning", "search", "find", "look", "definition", "language"],
    "rare": ["perspicacious", "sesquipedalian", "defenestration", "tmesis", "zugzwang", "callipygian", "petrichor"],
    "misspelled": ["tset", "wrod", "exmaple", "dictionery", "meanign", "serach", "fnid", "lokk"],
    "short": ["a", "i", "to", "be", "it", "is", "in", "on", "at", "by"],
    "long": ["antidisestablishmentarianism", "pneumonoultramicroscopicsilicovolcanoconiosis", "hippopotomonstrosesquippedaliophobia"],
}
...
async def benchmark_concurrent_load(concurrent_requests: int = 20, duration_seconds: int = 10) -> dict:
...
def display_results(results: list[dict]):
    ...
    if concurrent_result and "error" not in concurrent_result:
        ...
        console.print(f"  Errors: {concurrent_result['errors']} ({concurrent_result['error_rate']:.1%})")
    ...
    for result in results:
        if "error" not in result and result["search_type"] in targets:
            ...
            if p95 <= target:
                console.print(f"  ✅ {result['search_type']}: P95 = {p95:.2f}ms (target: <{target}ms)")
            else:
                console.print(f"  ❌ {result['search_type']}: P95 = {p95:.2f}ms (target: <{target}ms)")
