...
class BatchScheduler:
    ...
    async def run_batch_processing(self, word_limit: int | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            self._log(f"Processing parameters: {word_limit} words, batch size {batch_size}, estimated cost ${estimated_cost:.2f}")
            ...
            if result["status"] == "completed":
                ...
                self._log(f"Batch processing completed successfully: {result['total_words_processed']} words processed")
            else:
                self.state.failed_runs += 1
                error_msg = result.get("error", "Unknown error")
                if self.state.processing_errors is not None:
                    self.state.processing_errors.append(f"{datetime.now().isoformat()}: {error_msg}")
    ...
    def get_status_report(self) -> dict[str, Any]:
        ...
        status = {
            ...
            "state": {
                ...
                "success_rate": (self.state.successful_runs / max(1, self.state.successful_runs + self.state.failed_runs)) * 100
            },
            "current_status": {
            ...
            "recent_errors": self.state.processing_errors[-5:] if self.state.processing_errors else []
        }
