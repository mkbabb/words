...
@click.option("--types", default="multiple_choice,fill_blank", 
...
def create_deck(
    ...
    """
    asyncio.run(_create_deck_async(
        deck_name, input_file, words, output, types, max_cards, overwrite
    ))
    ...
    try:
        ...
        if not word_list:
            console.print(format_error(
                "No words provided",
                "Specify words using --input file or --words arguments."
            ))
            return
        ...
        # Show deck summary
        console.print(format_deck_summary(
...
async def _parse_word_file(file_path: str) -> list[str]:
    ...
    try:
        ...
        if path.suffix.lower() == '.md':
            ...
            numbered_items = re.findall(r'^[\s]*\d+\.[\s]*([a-zA-Z][a-zA-Z\s]*)', content, re.MULTILINE)
...
@click.argument("deck_name")
@click.option("--format", "export_format", type=click.Choice(["apkg", "html", "csv"]), 
