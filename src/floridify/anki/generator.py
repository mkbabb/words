    ...
    def _render_template(self, template: str) -> str:
        ...
        for field_name, field_value in self.fields.items():
            if not isinstance(field_value, list):  # Skip lists (already handled)
                result = result.replace(f"{{{{{field_name}}}}}", str(field_value) if field_value else "")
...
class AnkiCardGenerator:
    ...
    async def _generate_multiple_choice_card(
        ...
        try:
            ...
            fields = {
                ...
                "Synonyms": [syn.word.text for syn in definition.synonyms[:5]] if definition.synonyms else [],
            }
    ...
    async def _generate_fill_blank_card(
        ...
        try:
            ...
            complete_sentence = sentence_with_blank.replace(
                "_____", 
                f'<span class="word-highlight">{entry.word.text}</span>'
            )
    ...
    def _generate_html_preview(self, cards: list[AnkiCard], deck_name: str) -> str:
        ...
        html_parts = [
            ...
            f".card-container {{ margin-bottom: 40px; }}",
            f".card-side {{ margin-bottom: 20px; }}",
            f".card-label {{ font-weight: bold; margin-bottom: 10px; }}",
            f"</style>",
            f"</head><body>",
            f"<h1>{deck_name}</h1>",
        ...
        for i, card in enumerate(cards, 1):
            html_parts.extend([
                ...
                f"<div class='card-label'>Front:</div>",
                f"<style>{card.template.css_styles}</style>",
                card.render_front(),
                f"</div>",
                f"<div class='card-side'>",
                f"<div class='card-label'>Back:</div>",
                card.render_back(),
                f"</div>",
                f"</div>",
            ])
    ...
    def _create_genanki_model(self, card_type: CardType, template: AnkiCardTemplate) -> genanki.Model:
